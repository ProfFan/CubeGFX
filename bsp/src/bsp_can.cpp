...
int CAN::registerCallback(uint32_t messageID, std::function<void(CAN_RxMessageTypeDef*)> callback) {
...
void CAN::processFrame(CAN_RxMessageTypeDef* message) {
  for (int i = 0; i < tableSize; i++) {
    if ((idTable[i] == message->header.StdId) || (idTable[i] == 0)) {
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *_hcan) {
  ...
  if (_hcan == can1->hcan) {
    ...
    HAL_CAN_GetRxMessage(_hcan, CAN_RX_FIFO0, &msg.header, msg.data);
...
void StartCANBusTask(void const *argument) {
  ...
  CAN_FilterConfigStructure.FilterBank = 0; // Filter Bank 0-14 for CAN1
